...
public final class ConcurrentWeakHashMap<K, V> extends ConcurrentRefHashMap<K, V> {
  private static class WeakKey<K, V> extends WeakReference<K> implements KeyReference<K, V> {
    ...
    @NotNull private final ObjectHashingStrategy<K> myStrategy;
    private final V value;

    private WeakKey(@NotNull K k, final int hash, @NotNull ObjectHashingStrategy<K> strategy, V v, ReferenceQueue<K> q) {
      super(k, q);
      myStrategy = strategy;
      value = v;
      myHash = hash;
  ...
  @NotNull
  @Override
  protected KeyReference<K, V> createKeyReference(@NotNull K key, @NotNull V value, @NotNull ObjectHashingStrategy<K> hashingStrategy) {
  ...
  ConcurrentWeakHashMap(@NotNull ObjectHashingStrategy<K> hashingStrategy) {
